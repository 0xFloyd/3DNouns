...
const CouchHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/CouchHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Couch' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/CouchHead.glb');

export default CouchHead;
