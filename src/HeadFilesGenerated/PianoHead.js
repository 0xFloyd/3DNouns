...
const PianoHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadPiano.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Piano' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadPiano.glb');

export default PianoHead;
