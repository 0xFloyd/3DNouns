...
const HotdogHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HotDog.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Hotdog' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HotDog.glb');

export default HotdogHead;
