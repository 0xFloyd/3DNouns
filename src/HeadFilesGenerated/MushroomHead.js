...
const MushroomHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/mushroomHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Mushroom' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/mushroomHead.glb');

export default MushroomHead;
