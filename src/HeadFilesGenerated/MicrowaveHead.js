...
const MicrowaveHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/microwaveHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Microwave' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/microwaveHead.glb');

export default MicrowaveHead;
