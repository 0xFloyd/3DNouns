...
const FilmreelHead = ({
  ...
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadFilmReel.glb');
  ...
  return (
    <group
      ref={group}
      dispose={null}
      visible={headProp === 'Filmreel' ? true : false}
      castShadow
      receiveShadow
    >
      <primitive
...
useGLTF.preload('/headModels/HeadFilmReel.glb');

export default FilmreelHead;
