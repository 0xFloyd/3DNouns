...
const PopsodaHead = ({
  ...
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadPopSoda.glb');
  ...
  return (
    <group
      ref={group}
      dispose={null}
      visible={headProp === 'Popsoda' ? true : false}
      castShadow
      receiveShadow
    >
      <primitive
...
useGLTF.preload('/headModels/HeadPopSoda.glb');

export default PopsodaHead;
