...
const BagHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadBag.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Bag' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadBag.glb');
