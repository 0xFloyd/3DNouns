...
const CdHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/cdHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Cd' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/cdHead.glb');

export default CdHead;
