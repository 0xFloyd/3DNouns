...
const ChainHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadChain.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Chain' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadChain.glb');
