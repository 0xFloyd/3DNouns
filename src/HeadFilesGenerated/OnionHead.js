...
const OnionHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadOnion.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Onion' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadOnion.glb');

export default OnionHead;
