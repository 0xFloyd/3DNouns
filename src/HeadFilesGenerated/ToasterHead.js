...
const ToasterHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/toasterHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Toaster' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/toasterHead.glb');

export default ToasterHead;
