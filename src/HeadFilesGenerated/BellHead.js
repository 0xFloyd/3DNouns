...
const BellHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadBell.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Bell' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadBell.glb');

export default BellHead;
