...
const BoxinggloveHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadBoxingGlove.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Boxingglove' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadBoxingGlove.glb');

export default BoxinggloveHead;
