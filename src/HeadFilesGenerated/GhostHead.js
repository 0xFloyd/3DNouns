...
const GhostHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/ghostHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Ghost-B' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/ghostHead.glb');
