...
const MouseHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadMouse.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Mouse' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadMouse.glb');

export default MouseHead;
