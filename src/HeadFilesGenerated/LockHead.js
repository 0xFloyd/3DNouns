...
const LockHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadLock.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Lock' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadLock.glb');
