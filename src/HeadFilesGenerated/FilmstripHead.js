...
const FilmstripHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadFilmStrip.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Film-Strip' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadFilmStrip.glb');

export default FilmstripHead;
