...
const RabbitHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/rabbitHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Rabbit' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/rabbitHead.glb');
