...
const FirHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadFir.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Fir' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadFir.glb');

export default FirHead;
