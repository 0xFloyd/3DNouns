...
const PorkbaoHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/porkbaoHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Porkbao' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/porkbaoHead.glb');

export default PorkbaoHead;
