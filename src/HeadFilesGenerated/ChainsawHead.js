...
const ChainsawHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadChainsaw.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Chainsaw' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadChainsaw.glb');

export default ChainsawHead;
