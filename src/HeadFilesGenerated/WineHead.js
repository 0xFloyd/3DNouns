...
const WineHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadWine.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Wine' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadWine.glb');

export default WineHead;
