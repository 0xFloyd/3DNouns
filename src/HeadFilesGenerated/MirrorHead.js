...
const MirrorHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadMirror.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Mirror' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadMirror.glb');

export default MirrorHead;
