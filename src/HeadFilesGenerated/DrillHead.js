...
const DrillHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadDrill.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Drill' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadDrill.glb');

export default DrillHead;
