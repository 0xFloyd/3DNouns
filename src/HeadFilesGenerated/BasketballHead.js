...
const BasketballHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadBasketball.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Basketball' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadBasketball.glb');

export default BasketballHead;
