...
const FenceHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadFence.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Fence' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadFence.glb');

export default FenceHead;
