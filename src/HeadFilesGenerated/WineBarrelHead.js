...
const WineBarrelHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/WineBarrelHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Wine-Barrel' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/WineBarrelHead.glb');

export default WineBarrelHead;
