...
const IcepopHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/icepopHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Icepop-B' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/icepopHead.glb');

export default IcepopHead;
