...
const PaintbrushHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadPaintbrush.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Paintbrush' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadPaintbrush.glb');

export default PaintbrushHead;
