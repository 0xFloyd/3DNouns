...
const CrochatHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadCrocHat.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Croc-Hat' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadCrocHat.glb');

export default CrochatHead;
