...
const ChipboardHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadChipBoard.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Chipboard' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadChipBoard.glb');

export default ChipboardHead;
