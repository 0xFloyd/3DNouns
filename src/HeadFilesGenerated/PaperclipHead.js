...
const PaperclipHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/paperclipHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Paperclip' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/paperclipHead.glb');

export default PaperclipHead;
