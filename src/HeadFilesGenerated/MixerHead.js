...
const MixerHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/mixerHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Mixer' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/mixerHead.glb');

export default MixerHead;
