...
const JellyfishHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadJellyFish.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Jellyfish' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadJellyFish.glb');

export default JellyfishHead;
