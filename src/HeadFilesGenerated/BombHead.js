...
const BombHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadBomb.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Bomb' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadBomb.glb');

export default BombHead;
