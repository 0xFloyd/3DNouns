...
const SquidHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/squidHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Squid' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/squidHead.glb');

export default SquidHead;
