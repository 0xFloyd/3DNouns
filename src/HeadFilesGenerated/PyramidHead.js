...
const PyramidHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadPyramid.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Pyramid' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadPyramid.glb');

export default PyramidHead;
