...
const StaplerHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadStapler.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Stapler' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadStapler.glb');

export default StaplerHead;
