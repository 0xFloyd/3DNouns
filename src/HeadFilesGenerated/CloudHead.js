...
const CloudHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/cloudHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Cloud' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/cloudHead.glb');
