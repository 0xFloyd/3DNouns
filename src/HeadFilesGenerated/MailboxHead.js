...
const MailboxHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadMailbox.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Mailbox' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadMailbox.glb');
