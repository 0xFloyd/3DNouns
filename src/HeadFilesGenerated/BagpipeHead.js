...
const BagpipeHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadBagPipe.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Bagpipe' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadBagPipe.glb');

export default BagpipeHead;
