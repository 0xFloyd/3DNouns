...
const GrouperHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/grouperHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Grouper' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/grouperHead.glb');

export default GrouperHead;
