...
const MacaroniHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadMacaroni.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Macaroni' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadMacaroni.glb');

export default MacaroniHead;
