...
const NigiriHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/nigiriHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Nigiri' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/nigiriHead.glb');

export default NigiriHead;
