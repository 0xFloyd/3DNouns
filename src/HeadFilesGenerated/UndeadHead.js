...
const UndeadHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/undeadHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Undead' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/undeadHead.glb');

export default UndeadHead;
