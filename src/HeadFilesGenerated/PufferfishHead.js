...
const PufferfishHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadPufferFish.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Pufferfish' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadPufferFish.glb');

export default PufferfishHead;
