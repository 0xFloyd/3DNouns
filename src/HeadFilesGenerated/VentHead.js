...
const VentHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/Headvent.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Vent' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/Headvent.glb');

export default VentHead;
