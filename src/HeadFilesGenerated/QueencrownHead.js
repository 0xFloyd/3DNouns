...
const QueencrownHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/queencrownHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Queencrown' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/queencrownHead.glb');

export default QueencrownHead;
