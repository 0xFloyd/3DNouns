...
const BootHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/bootHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Boot' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/bootHead.glb');

export default BootHead;
