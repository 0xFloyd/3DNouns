...
const OrcaHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadOrca.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Orca' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadOrca.glb');

export default OrcaHead;
