...
const CookieHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadCookie.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Cookie' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadCookie.glb');
