...
const GnomeHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/gnomeHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Gnome' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/gnomeHead.glb');

export default GnomeHead;
