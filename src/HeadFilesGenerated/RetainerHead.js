...
const RetainerHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadRetainer.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Retainer' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadRetainer.glb');

export default RetainerHead;
