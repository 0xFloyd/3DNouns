...
const FrogHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/frogHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Frog' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/frogHead.glb');
