...
const CamcorderHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadCamcorder.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Camcorder' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadCamcorder.glb');

export default CamcorderHead;
