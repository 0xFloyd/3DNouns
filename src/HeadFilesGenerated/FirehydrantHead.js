...
const FirehydrantHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadFireHydrant.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Firehydrant' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadFireHydrant.glb');

export default FirehydrantHead;
