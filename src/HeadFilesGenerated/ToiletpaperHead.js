...
const ToiletpaperHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadToiletPaper.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Toiletpaper-Full' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadToiletPaper.glb');

export default ToiletpaperHead;
