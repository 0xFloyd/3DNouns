...
const MazeHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadMaze.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Maze' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadMaze.glb');

export default MazeHead;
