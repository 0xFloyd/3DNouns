...
const ChipsHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadChips.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Chips' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadChips.glb');

export default ChipsHead;
