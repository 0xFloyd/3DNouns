...
const OutletHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadOutlet.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Outlet' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadOutlet.glb');
