...
const PillHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadPill.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Pill' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadPill.glb');

export default PillHead;
