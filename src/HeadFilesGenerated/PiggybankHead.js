...
const PiggybankHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadPiggyBank.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Piggybank' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadPiggyBank.glb');

export default PiggybankHead;
