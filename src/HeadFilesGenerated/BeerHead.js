...
const BeerHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/beerHead.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Beer' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/beerHead.glb');
