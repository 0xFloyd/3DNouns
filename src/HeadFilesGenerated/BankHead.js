...
const BankHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadBank.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Bank' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadBank.glb');
