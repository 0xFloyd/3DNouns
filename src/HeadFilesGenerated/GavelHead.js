...
const GavelHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadGavel.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Gavel' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadGavel.glb');

export default GavelHead;
