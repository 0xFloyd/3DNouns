...
const ToothHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/HeadTooth.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Tooth' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/HeadTooth.glb');

export default ToothHead;
