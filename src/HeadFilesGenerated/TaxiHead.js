...
const TaxiHead = ({ headProp, glassesProp, animationState, animationValue, masterHeadModel }) => {
  const group = useRef();
  const { nodes, materials } = useGLTF('/headModels/Headtaxi.glb');
  ...
  return (
    <group ref={group} dispose={null} visible={headProp === 'Taxi' ? true : false} castShadow receiveShadow>
...
// useGLTF.preload('/headModels/Headtaxi.glb');

export default TaxiHead;
