...
const NormalEnvironment = ({ environment }) => {
  ...
  return (
    <>
      {environment === 'Normal' || environment === 'VoidDay' ? <fog attach="fog" args={[new THREE.Color(0xffffff), 1, 2500]} /> : null}
      {environment === 'Matrix' ? <fog attach="fog" args={[new THREE.Color(0x181818), 1, 2500]} /> : null}
      {environment === 'VoidNight' ? <fog attach="fog" args={[new THREE.Color(0x181818), 1, 1000]} /> : null}
      ...
      {loaded && (environment === 'Matrix' || environment === 'VoidNight') ? <color attach="background" args={[0x181818]} /> : null}
      {loaded && (environment === 'Normal' || environment === 'VoidDay') ? (
        <Sky azimuth={0.5} turbidity={7.5} rayleigh={0.4} inclination={0.6} distance={3000} sunPosition={[-100, 500, 1000]} />
      ) : null}
      ...
      {environment === 'Normal' || environment === 'Matrix' ? (
        <>
          <group dispose={null} scale={[140, 140, 140]} position={[0, 1.5, 100]}>
            {' '}
