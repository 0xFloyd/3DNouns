...
const starterComponent = fs.readFileSync('./HeadStarterComponent.js').toString();
...
jsonData.head.forEach((headData) => {
  ...
  let replaceComponentName = starterComponent.replace(/HeadComponent/g, headData.componentName);

  let replaceFileName = replaceComponentName.replace(/filePlaceholderName.glb/g, headData.filePath);
  let replaceHeadName = replaceFileName.replace(/HeadVariableName/g, `${headData.name}`);

  // write starter component to file
  fs.writeFileSync(`${writeDirectory}/${filePath}`, replaceHeadName, function (err) {
